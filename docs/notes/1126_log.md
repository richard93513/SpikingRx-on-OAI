...
<img width="857" height="1850" alt="Untitled diagram-2025-11-26-174242" src="https://github.com/user-attachments/assets/27ee35ed-93f7-4bc9-af82-07e3aa2adc6f" />
...
// =====================================================
{
    ...
    // frame, slot 是這個 function 的參數
    int frame_tx = frame;
    int slot_tx  = slot;

    // 建立資料夾結構（與 fullgrid 一致）
    const char *root = "/home/richard93513/SpikingRx-on-OAI/spx_records/raw";
    mkdir("/home/richard93513/SpikingRx-on-OAI", 0755);
    mkdir("/home/richard93513/SpikingRx-on-OAI/spx_records", 0755);
    mkdir("/home/richard93513/SpikingRx-on-OAI/spx_records/raw", 0755);
    ...
    char fname[512];
    snprintf(fname, sizeof(fname),
             "%s/f%04d_s%02d_txbits_idx%06u_rnti%05d.bin",
             root,
             frame_tx,
             slot_tx,
             spx_dump_idx,
    ...
    if (!fp2) {
        LOG_E(PHY, "[SPX][ERR] Cannot open %s for TX TB dump\n", fname);
        goto SPX_TXTB_END;
    }

    size_t tb_bytes = A >> 3;   // A bits → bytes
    fwrite(a, 1, tb_bytes, fp2);
    fclose(fp2);

    LOG_I(PHY,
          "[SPX] TXTB dump: frame=%d slot=%d  rnti=%d  bytes=%ld → %s  (idx=%u)\n",
          frame_tx, slot_tx, rel15->rnti, tb_bytes, fname, spx_dump_idx);
...
https://github.com/richard93513/SpikingRx-on-OAI/blob/main/src/data/oai_to_spikingrx_tensor.py
...
https://github.com/richard93513/SpikingRx-on-OAI/blob/main/src/inference/run_spikingrx_on_oai_dump.py
...
https://github.com/richard93513/SpikingRx-on-OAI/blob/main/src/tools/bundle_records.py
...
# 🟩 10.如何完成 SpikingRx vs OAI BER 比較（最精簡版）

整個流程只有 4 步。

---

## ✅ Step 1 — 準備資料：fullgrid + txbits

你已完成：

- RX fullgrid → `fullgrid.bin`  
- TX TB bits（ground truth）→ `txbits.bin`  
- bundle → `fXXXX_sYY/` 資料夾  

這兩個就是 **BER 必備的成對資料**。

---

## ✅ Step 2 — SpikingRx 產生 LLR

使用你的 inference pipeline：

```
fullgrid → Tensor → SpikingRx → LLR float → LLR int8
```

輸出：

```
llr_int8_for_oai.bin   ← 給 LDPC decoder 用
```

---

## ✅ Step 3 — 用 OAI LDPC Decoder 解碼

你會寫一個 Python 版：

```python
decoded_bits = oai_ldpc_decode(llr_int8, config)
```

config 從 `meta.json` 或 OAI 決定。

重點：

- **LLR（int8）進 decoder**  
- **decoder 產生 decoded bits**  

---

## ✅ Step 4 — 比對 txbits，計算 BER

```python
bit_errors = sum(decoded_bits != txbits)
ber = bit_errors / len(txbits)
```

這就是 **SpikingRx 的 BER**。

> 之後 baseline 也用同樣 decoder → 可以直接比較。
...
```
LLR → LDPC decoder → decoded bits → compare with txbits
```

這就是完整、正確的 **SpikingRx BER pipeline**。
