                                   ...
                                   fapi_nr_dci_indication_pdu_t *dci_ind)
{
  ...
  if (rnti_type == TYPE_RA_RNTI_) {
    ...
    if (!get_FeedbackDisabled(mac->sc_info.downlinkHARQ_FeedbackDisabled_r17, dci->harq_pid.val)) {
      ...
      // [SPX] RFsim workaround: 不要因為 HARQ timing 太短就整個 UE 掛掉
      if (feedback_ti < GET_DURATION_RX_TO_TX(&mac->ntn_ta, dlsch_pdu->SubcarrierSpacing)) {
        LOG_W(NR_MAC,
              "[SPX] Skip HARQ timing assert in RFsim (feedback_ti=%d, duration_rx_to_tx=%ld)\n",
              feedback_ti,
              GET_DURATION_RX_TO_TX(&mac->ntn_ta, dlsch_pdu->SubcarrierSpacing));
        // 這裡不 return、不 exit，繼續照常 set_harq_status()
      }
