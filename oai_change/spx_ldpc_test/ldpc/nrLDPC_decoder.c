...
// local forward typedef so we can use it in stubs
typedef void decode_abort_t;

// --- standalone stub for OAI global profiling flag ---
int cpu_meas_enabled = 0;

// --- Standalone stubs: logging & abort (no-op) ---
#ifndef LOG_D
#define LOG_D(cat, fmt, ...) do{}while(0)
#endif
...
static inline void set_abort(decode_abort_t *ab, bool flag) {
  (void)ab;
  (void)flag;
}
