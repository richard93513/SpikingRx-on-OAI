...
// ==== Added for standalone LDPC decoder (remove OAI task dependency) ====
typedef void task_ans_t;
typedef void decode_abort_t;
typedef void (*LDPC_decoderfunc_t)(void);
...
*/
typedef int32_t nrLDPC_decoder(t_nrLDPC_dec_params *p_decParams,
                       int8_t *p_llr,
                       int8_t *p_out,
                       time_stats_t *p_profiler,
                       decode_abort_t *ab);
