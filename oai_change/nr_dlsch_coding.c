...
// ======== SPX: TX TB bits global index ========
static uint32_t spx_txbits_idx = 0;
                      ...
                      time_stats_t *dlsch_segmentation_stats)
{
  ...
  for (int i = 0; i < n_dlsch; i++) {
    ...
    unsigned char *a = dlsch->pdu;        // TB bits pointer

    // ==========================================================
    // SPX: Dump TX TB bits (after random, before CRC/LDPC)
    // ==========================================================
    {
      ...
      snprintf(fname,
               ...
               "f%04d_s%02d_txbits_idx%06u_rnti%05d.bin",
               home,
               frame,
               slot,
               spx_txbits_idx,
      ...
      size_t tb_bytes = (size_t)A >> 3;  // A bits → A/8 bytes
      ...
      if (fp2) {
        ...
        LOG_I(PHY,
              "[SPX] Dumped TX TB bits: frame=%d slot=%d rnti=%05d size=%ldB → %s (idx=%06u)\n",
              frame,
              slot,
              rel15->rnti,
              (long)tb_bytes,
        ...
        LOG_W(PHY, "[SPX] Failed to open TX TB dump file: %s\n", fname);
      }

      spx_txbits_idx++;  // 每個 TB dump 完自動 +1
    }
    // ---------------- END SPX dump TX TB bits -------------------
    ...
    if (A > NR_MAX_PDSCH_TBS) {
      ...
      memcpy(dlsch->b, a, (A / 8) + 4); // CRC 24 → 3 bytes + padding
    } else {
      ...
      memcpy(dlsch->b, a, (A / 8) + 3); // mimic 24-bit CRC case
    }

    nrLDPC_TB_encoding_parameters_t *TB_parameters = &TBs[i];

    // harq_unique_pid: use i (not actual HARQ pid)
  ...
  for (int i = 0; i < n_dlsch; i++) {
    ...
    // =====================================================
    // SPX: Dump LDPC config (JSON, per frame/slot)
    // =====================================================
    {
      const char *home = getenv("HOME");
      if (!home)
      ...
      if (fj) {
        ...
        LOG_I(PHY,
              "[SPX] Dumped LDPC cfg: frame=%d slot=%d dlsch_id=%d → %s\n",
              frame,
              slot,
              i,
              json_name);
        ...
        LOG_W(PHY,
              "[SPX] Failed to open LDPC cfg file: %s\n",
              json_name);
      }
    }
    ...
    // output for each dlsch aligned on 64-byte boundaries
